...
Rendered by go-mermaid (PNG at 2x) from the fixtures in `testdata/golden`.
Regenerate them after any rendering change with
`go run ./internal/gallery/cmd`. CI checks that each image still matches the
...
- [x] README examples gallery (sample PNG per diagram type), regenerated by
      `./internal/gallery/cmd` and drift-checked in CI
